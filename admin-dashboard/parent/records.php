...
$parentResult = $conn->query("SELECT Parent_ID, 
                                ...
                                Parent_Age,
                                Civil_Status_Name,
                                CONCAT(Street_Number, ' ', Barangay_Name, ' ', Town_Name, ' ', Province_Name, ' ', Zipcode) AS CompleteAddress,
                                Parent_Username,
                                Parent_Password
                          FROM parent_table
                          ...
                          INNER JOIN province_table ON parent_table.Province_ID = province_table.Province_ID");
...
<body>
  ...
  <section id="wrapper">
    ...
    <div class="p-4">
      <div class="welcome">
        <h1 class="fs-3">Parent Records</h1>
      </div>
    </div>
    ...
    <div class="mx-4">
      <table id="example" class="table table-striped display nowrap" style="width:100%" style="width:100%">
        ...
        <tbody>
          <?php if ($parentResult->num_rows > 0): ?>
          <?php foreach($parentResult as $row): ?>
          <tr>
            <td><?= $row['Parent_ID'] ?></td>
            <td><?= $row['ParentName']  ?></td>
            <td><?= $row['Parent_Birthdate'] ?></td>
            <td><?= $row['Gender_Name'] ?>
            </td>
            <td><?= $row['Parent_Age'] ?></td>
            <td><?= $row['Civil_Status_Name'] ?></td>
            <td><?= $row['CompleteAddress'] ?></td>
            <td><?= $row['Parent_Username'] ?></td>
            <td><?= $row['Parent_Password'] ?></td>
            <td>
              <button type="button" class="btn btn-primary editParentBtn" data-bs-toggle="modal"
                data-bs-target="#editModal" data-edit-id="<?= $row['Parent_ID'] ?>">
                <i class="fa-regular fa-pen-to-square"></i>
              </button>
            </td>
            <td>
              <button type="button" class="btn btn-danger deleteParentButton" data-bs-toggle="modal"
                data-bs-target="#deleteModal" data-delete-id="<?= $row['Parent_ID'] ?>">
          ...
          <div class="modal-content" style="width: 750px; background-color: #eee;">
            ...
            <div class="modal-body">
              <form id="editForm" class="row g-3">
                <input type="hidden" class="form-control" id="updateParentID" name="updateParentID">
                <div class="col-md-3">
                ...
                <div class="col-md-9 d-flex align-items-center mt-5">
                  <button type="submit" class="btn btn-success ms-auto me-2 px-4" id="updateParentBtn"
                    name="editParentBtn">Update Parent</button>
                </div>
              </form>
            </div>
          </div>
